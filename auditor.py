...
def est_un_vrai_prospect(url):
    """Filtre les annuaires et réseaux sociaux"""
    url_lower = url.lower()
    return not any(plateforme in url_lower for plateforme in SITES_A_IGNORER)

def check_advanced_seo(url):
    """Analyse complète du H1, Title et Meta Description avec contrôle de longueur"""
    results = {
        "title": "Manquant",
        "title_len": 0,
    ...
    try:
        # allow_redirects=True et verify=False si problème de certificat SSL local
...
def get_pagespeed_metrics(url, api_key=None):
    """Récupère les scores PageSpeed SEO et Performance"""
...
def audit_prospect(url, api_key=None):
    """Fonction principale d'audit"""
    target_url = clean_url(url)
    print(f" 🔍 Audit en cours : {target_url}...")
    
    # Désactiver les avertissements SSL en console si verify=False
    import urllib3
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

    seo_data = check_advanced_seo(target_url)
    
    # Si le site est vraiment inaccessible
    if seo_data["error"]:
        return {
            "url": target_url,
            "has_site": "Oui",
